...
namespace AppNary.Domain.UnitTest.Recipes.Commands.Handlers
{
    public class UpdateRecipeCommandHandlerTests
    {
        ...
        {
            FluentActions.Invoking(() => new UpdateRecipeCommandHandler(default(IRecipeRepository), _unitOfWorkMock.Object, _userAccessorManager.Object)).Should().Throw<ArgumentNullException>();
        }

        [Fact]
        public void Should_throw_when_creating_with_null_unit_of_work()
        {
            FluentActions.Invoking(() => new UpdateRecipeCommandHandler(_recipeRepositoryMock.Object, default(IUnitOfWork), _userAccessorManager.Object)).Should().Throw<ArgumentNullException>();
        }

        [Fact]
        public void Should_throw_when_creating_with_null_user_accessor_manager()
        {
            FluentActions.Invoking(() => new UpdateRecipeCommandHandler(_recipeRepositoryMock.Object, _unitOfWorkMock.Object, default(IUserAccessorManager))).Should().Throw<ArgumentNullException>();
        }
        ...
        public async void Should_throw_when_any_ingredient_does_not_exists()
        {
            ...
            var handler = new UpdateRecipeCommandHandler(_recipeRepositoryMock.Object, _unitOfWorkMock.Object, _userAccessorManager.Object);

            _recipeRepositoryMock.Setup(x => x.GetRecipe(It.IsAny<Guid>())).ReturnsAsync(_recipeFaker.Generate());
            ...
            // When
            await FluentActions.Invoking(() => handler.Handle(new UpdateRecipeCommandRequest { Name = recipe.Name, Description = recipe.Description, Ingredients = new List<RecipeIngredientQueryRequestDto> { ingredientDto } }, CancellationToken.None)).Should().ThrowAsync<DomainException>();
        ...
        public async void Should_handle()
        {
            ...
            // When
            var result = await handler.Handle(new UpdateRecipeCommandRequest { Name = recipe.Name, Description = recipe.Description, Ingredients = new List<RecipeIngredientQueryRequestDto> { ingredientDto } }, CancellationToken.None);

            // Then
            _userAccessorManager.Verify(x => x.ThrowIfUserDontHasAccess(It.IsAny<Recipe>()));
            _recipeRepositoryMock.Verify(x => x.Update(It.Is<Recipe>(y => y.Name == recipe.Name && y.Description == recipe.Description && y.Ingredients.First().IngredientId == ingredient.Id)));
